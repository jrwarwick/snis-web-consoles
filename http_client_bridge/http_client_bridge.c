...
//
//    https://smcameron.github.io/space-nerds-in-space/#speech-recognition
...
#define SNIS_SOUNDS_PATH "/usr/local/share/snis/sounds/"
...
static void fn(struct mg_connection *c, int ev, void *ev_data, void *fn_data) {
	...
	if (ev == MG_EV_ACCEPT && fn_data != NULL) {
		...
		if (mg_http_match_uri(hm, "/CMD/QUIT")) {
			...
			// copy string, strip /SHIP/ prefix, uppercase, remove anything not [A-Z/], replace '/' with ' ', send to NL fifo.
